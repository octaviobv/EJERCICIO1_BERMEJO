...
function setup() {
  ...
  // Conectar Socket.IO
  socket = io();
  socket.on('remote-press', (index) => {
    handleRemotePress(index);
  });

  // Cargar audios desde la carpeta /audio
...
function mousePressed() {
  for (let i = 0; i < buttons.length; i++) {
    ...
    if (dist(mouseX, mouseY, btn.x, btn.y) < btn.size / 2) {
      if (sounds[i]) {
        playSoundAt(i);
        // Notificar al servidor para que otros clientes reproduzcan
        if (socket && socket.connected) socket.emit('button-pressed', i);
      }
    }
  }
  return false;
...
function handleRemotePress(index) {
  // Reproducir localmente cuando otro cliente pulsa
  playSoundAt(index);
}
